...
def wishlist(request):
    products = Product.objects.filter(wished_item=request.user)
...
def user_orders(request):
    ...
    orders = Order.objects.filter(customer_id=user_id).filter(complete=True)
    return render(request, "cart/customer_orders.html", {"orders": orders})